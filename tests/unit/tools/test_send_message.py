"""Tests for longline.tools.send_message.send_message_tool — SendMessageTool."""
...
from longline.swarm.identity import format_agent_id
from longline.swarm.mailbox import TeammateMailbox
from longline.swarm.team_file import TeamFile, TeamMember, save_team_file
...
async def test_send_message(tool: SendMessageTool, team_dir: Path) -> None:
    ...
    with patch("longline.swarm.mailbox._DEFAULT_CLAUDE_DIR", team_dir):
...
async def test_broadcast(tool: SendMessageTool, team_dir: Path) -> None:
    ...
    with (
        patch("longline.swarm.mailbox._DEFAULT_CLAUDE_DIR", team_dir),
        patch("longline.swarm.team_file._DEFAULT_CLAUDE_DIR", team_dir),
    ):
        result = await tool.execute(
