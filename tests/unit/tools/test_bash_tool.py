...
class TestBashTool:
    ...
    async def test_cwd(self, tmp_path: object) -> None:
        ...
        # Cross-platform assert: POSIX pwd prints the real path, Windows/MSYS
        # maps it to /tmp/...; only the final path component is stable on both.
        assert tmp_path.name in result.content  # type: ignore[attr-defined]
