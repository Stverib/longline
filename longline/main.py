...
# --- 内核层 ---
from longline.api.claude import stream_response          # provider adapter: 把 Anthropic SDK 的流式事件转为内部 QueryEvent
from longline.api.client import create_client             # Anthropic SDK 客户端工厂
from longline.core.events import QueryEvent, TurnComplete # 内核事件类型（query_loop 的产出物）
from longline.core.query_engine import QueryEngine        # 运行时依赖容器，封装 main → query_loop 的中间层

# --- 装配层 ---
from longline.hooks.hook_runner import load_hooks         # 从 settings.json 加载 hook 配置
from longline.models.messages import Message, UserMessage  # transcript 的基本元素
from longline.prompts.builder import build_system_prompt   # system prompt 拼装器（多段合并）
...
from longline.tools.base import ToolRegistry               # 工具注册表：name → Tool 实例的映射 + schema 生成
...
def _build_registry(cwd: str, call_model_factory: object | None = None, model: str = "") -> ToolRegistry:
    ...
    from longline.tools.ask_user.ask_user_tool import AskUserQuestionTool

    registry.register(AskUserQuestionTool(input_fn=True))

    from longline.tools.todo.todo_write_tool import TodoWriteTool
    ...
    from longline.tools.tool_search.tool_search_tool import ToolSearchTool
    ...
    from longline.skills.loader import load_skills as _load_skills
    from longline.tools.skill.skill_tool import SkillTool

    skills = _load_skills(cwd)
    registry.register(SkillTool(skills=skills))

    # PlanMode tools — enter/exit plan mode
    from longline.tools.plan_mode.plan_mode_tool import EnterPlanModeTool, ExitPlanModeTool

    registry.register(EnterPlanModeTool())
    registry.register(ExitPlanModeTool())

    # BriefTool — conversation summary stats
    from longline.tools.brief.brief_tool import BriefTool

    registry.register(BriefTool())

    # LSPTool — language server protocol integration (stub)
    from longline.tools.lsp.lsp_tool import LSPTool

    registry.register(LSPTool())

    # --- Tier 3: 协作工具（Team / Swarm）---
    # 这些工具在这里只做「骨架注册」，_build_engine() 会做二次布线注入 team_context
    from longline.tools.send_message.send_message_tool import SendMessageTool
    from longline.tools.team.team_create_tool import TeamCreateTool
    from longline.tools.team.team_delete_tool import TeamDeleteTool
...
async def _connect_mcp_servers(cwd: str, registry: ToolRegistry) -> None:
    ...
    配置来源: ~/.longline/mcp.json + 项目级 .mcp.json
    每个 MCP server 连接后，其工具会被动态注册到 registry 中，
    从此与内置工具一视同仁地参与 query_loop 的工具调用流程。
    """
    from longline.mcp.client import connect_mcp_server
    from longline.mcp.config import load_mcp_configs
...
def _build_system(cwd: str, model: str, claude_md: str | None) -> str:
    ...
    """
    from longline.memory.session_memory import get_memory_dir, load_memory_index

    # memory_dir 路径: ~/.longline/projects/<sha256(cwd)[:12]>/memory/
...
def _build_engine(
    ...
    *,
    ...
    hooks = load_hooks()                  # 从 ~/.longline/settings.json 读取 hook 配置

    # === Step 2: Coordinator 模式 ===
    # 如果设置了环境变量 CC_COORDINATOR=1，会在 system prompt 前面追加
    # coordinator 专用指令（教模型如何分配任务给 teammates）
    from longline.swarm.coordinator import maybe_inject_coordinator_prompt
    ...
    # === Step 4: 运行时基础设施 ===
    from longline.session.task_registry import TaskRegistry as _TaskRegistry
    from longline.tools.agent.background import BackgroundAgentManager
    ...
    from longline.swarm.team_context import TeamContext
    ...
    from longline.tools.agent.agent_tool import AGENT_TOOL_NAME, AgentTool
    ...
    from longline.tools.task_tools.task_tools import TaskStopTool
...
async def _run_repl(
    ...
    if skills:
        from longline.commands.registry import register_command
    ...
    from longline.memory.extractor import ExtractionCoordinator

    extraction_coord = ExtractionCoordinator()

    # === Resume 恢复流程 ===
    # 从 ~/.longline/sessions/<session_id>.jsonl 加载旧 transcript
    ...
    from longline.ui.renderer import print_welcome
    ...
    while True:
        ...
        if user_input.strip().startswith("/"):
            from longline.commands.registry import get_command, parse_slash_command
            ...
            if result == "__CLEAR__":
                ...
                from longline.commands.registry import DASHSCOPE_BASE_URL, is_dashscope_model
                ...
                from longline.skills.loader import get_skill_by_name
        ...
        # 记录到输入历史（~/.longline/history.jsonl），用于 session 列表展示
...
def main(
    ...
    两种运行模式：
      1. print mode (-p): echo "prompt" | python -m longline -p → 单次输出后退出
