...
def is_coordinator_mode() -> bool:
    """Check if LONGLINE_COORDINATOR_MODE env var is set."""
    # 通过环境变量判断是否启用协调器模式
    # 协调器模式下，主 agent 会作为"组长"角色分配任务给子 agent
    val = os.environ.get("LONGLINE_COORDINATOR_MODE", "")
...
def get_coordinator_config() -> dict[str, object]:
    ...
    return {
        ...
        "env_var": "LONGLINE_COORDINATOR_MODE",
    }
...
def maybe_inject_coordinator_prompt(base_prompt: str) -> str:
    ...
    from longline.prompts.coordinator_prompt import get_coordinator_prompt
